...
`agent-server-example` enables its optional `model-management` Cargo feature by
...
Management endpoints live in `examples/server/src/model_settings.rs`, separately
...
The first startup seeds from existing `AGENT_MODEL_ENDPOINT`, `AGENT_MODEL_NAME`,
`AGENT_MODEL_KEY`, and `AGENT_MODEL_EXTRA_JSON`. Subsequent starts use the saved document.
Set `AGENT_MODEL_STORE_KEY` to a stable random secret of at least 16 characters.
For existing local development setups only, omission falls back to `AGENT_MODEL_KEY`;
changing that environment key will then prevent decryption. Set a dedicated store key
before first use if provider credentials will rotate.
...
The development launcher continues to pass existing environment variables and only
serves the bridge connection token; it does not manage model catalogs or keys. The
Web UI separately discovers the host management API. Tauri hosts can implement the
same operations using the manager without depending on HTTP; no new native commands
