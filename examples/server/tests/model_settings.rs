...
use agent_api::{AgentExecutor, Result, RunRequest, RunStatus};
use agent_core::HostBuilder;
use agent_model_manager::{ModelManager, SettingsStore};
use axum::{
    body::Body,
...
async fn changing_default_during_a_tool_run_does_not_change_its_next_round() {
    tokio::time::timeout(Duration::from_secs(5), async {
        ...
        let server = tokio::spawn(async move {
            ...
            for index in 0..3 {
                ...
                let chunk = if index == 0 {
                    ...
                    changing_manager.set_default(1, agent_model_manager::Selection {
        ...
        let mut host = HostBuilder::new().plugin(manager.plugin()).tool(Arc::new(agent_demo::Add)).build().await.unwrap();
        let runtime = manager.runtime(Arc::new(host.engine()));
