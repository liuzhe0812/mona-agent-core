use agent_api::{AgentError, ErrorCode, EventEnvelope, RunOutcome, RunSnapshot};
...
pub(crate) fn public_event(mut event: EventEnvelope) -> EventEnvelope {
    if let agent_api::RunEvent::RunFinished { outcome } = &mut event.event { public_outcome(outcome); }
    event
}
