...
function fixture({items=frames(),idle=false,completed=false}={}){
 ...
 const invoke=async(command,args)=>{const name=command.split('|')[1];calls.push({name,args});switch(name){
...
test('both transport clients use the same application request shape',async()=>{const f=fixture();await f.client.start({request_id:'key',prompt:'hello'});assert.deepEqual(f.calls[0],{name:'start_task',args:{request:{request_id:'key',prompt:'hello'}}});});
