...
async fn scopes_refresh_without_elevating_rules_or_mistaking_them_for_user_turns() {
    ...
    let rules = ProjectInstructions::new(root, None).unwrap();
    let ctx = context();
...
async fn corrupt_oversized_and_cancelled_rules_fail_explicitly() {
    let temp = tempfile::tempdir().unwrap();
    let rules = ProjectInstructions::new(temp.path(), None).unwrap();
    let ctx = context();
...
struct ChangingRulesModel { calls: AtomicUsize, root: PathBuf }
#[async_trait]
impl Model for ChangingRulesModel {
    async fn stream(&self, request: ModelRequest, _: CancellationToken) -> Result<ModelStream> {
        ...
        let call = |index, id: &str, path: &str, content: &str| ModelEvent::ToolDelta {
            index, id:Some(id.into()), name:Some("write".into()),
            arguments:json!({"path":path,"content":content}).to_string(),
        };
        let events = match round {
            0 => vec![call(0,"new-rules","AGENTS.md","RULE_UPDATED: reconsider target.txt."),
                call(1,"stale-target","target.txt","must not be written"),
                ModelEvent::Finish(FinishReason::ToolCalls), ModelEvent::End],
            1 => {
                assert!(!self.root.join("target.txt").exists(), "stale same-batch action was dispatched");
                assert!(serde_json::to_string(&request.messages).unwrap().contains("RULE_UPDATED"));
                assert!(request.messages.iter().any(|m| matches!(m, Message::Tool { result }
                    if result.call_id == "stale-target" && result.status == ToolStatus::Denied)));
                vec![call(0,"fresh-target","target.txt","reconsidered"),
                    ModelEvent::Finish(FinishReason::ToolCalls), ModelEvent::End]
            },
            2 => vec![ModelEvent::Text("done".into()),ModelEvent::Finish(FinishReason::Stop),ModelEvent::End],
...
impl CheckpointSink for SavePolicyResults {
    async fn commit(&self, cp: Arc<RunCheckpoint>, _: CancellationToken) -> Result<()> {
        self.0.lock().unwrap().push((*cp).clone()); Ok(())
    }
}
#[tokio::test]
async fn same_batch_rule_change_denies_stale_write_then_allows_fresh_call_with_durable_results() {
    ...
    let rules = Arc::new(ProjectInstructions::new(temp.path(), None).unwrap());
    ...
    let mut host = runtime::HostBuilder::new()
        .model(Arc::new(ChangingRulesModel { calls:AtomicUsize::new(0), root:temp.path().into() }))
        .context_transform(rules.clone()).policy(rules)
        .tool(Arc::new(tools::WriteTool::new(config))).allow_side_effect_tool("write")
        .checkpoint_sink(Arc::new(SavePolicyResults(checkpoints.clone()))).unwrap().build().await.unwrap();
    let handle = host.engine().start(RunRequest::new("change rules and reconsider target")).unwrap();
    let mut events = handle.subscribe();
    let report = handle.wait().await.unwrap();
    assert_eq!(report.status, RunStatus::Completed, "{:?}", report.error);
    assert_eq!(fs::read_to_string(temp.path().join("target.txt")).unwrap(), "reconsidered");
    assert_eq!(report.transcript.iter().filter(|m| matches!(m, Message::Tool { result }
        if result.call_id == "new-rules" && result.status == ToolStatus::Success)).count(), 1);
    assert!(checkpoints.lock().unwrap().iter().any(|cp| matches!(cp.pending_tools.get("stale-target"),
    ...
    while let Ok(event) = events.try_recv() {
        if let RunEvent::ItemCompleted { item } = event.event {
            if matches!(item.content, ItemContent::ToolCall { call_id:Some(ref id), .. } if id == "stale-target") {
                assert_eq!(item.state, ItemState::Denied); ended += 1;
            }
        }
    }
    assert_eq!(ended, 1);
