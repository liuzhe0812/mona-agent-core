...
这组文档说明 Mona 当前实现的架构、执行契约和二次开发边界，与所在提交的代码一起阅读。Mona 分为**核心层、扩展层、产品层（Web）**；核心可以独立嵌入，使用 Web 不是接入前提。
...
| 修改 Web、替换前端或接入 Tauri | [产品层与 Web 装配](architecture/WEB.zh-CN.md) | [核心执行机制：事件和状态](architecture/RUNTIME.zh-CN.md#events) |
...
| [产品层与 Web 装配](architecture/WEB.zh-CN.md) | Server、Application、Bridge、Client 和页面如何连接？怎样更换宿主而不复制执行器？ |
...
| 对象 | 当前值 | 权威定义 |
|---|---|---|
| Rust 扩展契约 | `API_VERSION = 9` | [`api/src/lib.rs`](../packages/api/src/lib.rs) |
| UI 事件协议 | `STREAM_VERSION = 2` | [`api/src/event.rs`](../packages/api/src/event.rs) |
| 运行检查点 | `CHECKPOINT_VERSION = 1` | [`api/src/checkpoint.rs`](../packages/api/src/checkpoint.rs) |
| Sessions 会话文件 | 格式 `3` | [`sessions/src/store.rs`](../packages/sessions/src/store.rs) |
