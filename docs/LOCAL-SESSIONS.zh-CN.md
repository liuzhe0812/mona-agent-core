...
每个文件严格包含两行 JSON。只支持当前格式 2，保存身份、工作空间、revision、导航标记、回合元数据、完整档案、工作前缀、最新检查点与可验证摘要。导航字段是当前格式的必填字段。已删除格式 1 读取和自动迁移；未知格式或不一致内容拒绝读取，不重置数据。完整档案不被摘要覆盖，新 Run 使用验证后的工作集。详见[上下文管理](CONTEXT-MANAGEMENT.zh-CN.md)。
