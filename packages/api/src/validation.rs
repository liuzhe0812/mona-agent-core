//! Shared message invariants for execution and persisted-history admission.
use crate::{AgentError, ErrorCode, Message, Result};
        ...
        match message {
            ...
            Message::Assistant {
                ...
                for call in tool_calls {
                    if let Some(data) = &call.provider_data {
                        data.validate()?;
                    }
                }
            }
        ...
        match message {
            Message::Tool { result } => {
                if !pending.remove(&result.call_id) {
                    return Err(AgentError::new(
                        ErrorCode::ModelProtocol,
                        "orphaned or duplicated tool result",
                    ));
                }
            }
            Message::Assistant { tool_calls, .. } => {
                if !pending.is_empty() {
                    return Err(AgentError::new(
                        ErrorCode::ModelProtocol,
                        "assistant message before tool batch settled",
                    ));
                }
                for call in tool_calls {
                    if call.id.is_empty() || !seen.insert(call.id.clone()) {
                        return Err(AgentError::new(
                            ErrorCode::ModelProtocol,
                            "empty or repeated tool-call id",
                        ));
                    }
            ...
            Message::System { .. } | Message::User { .. } => {
                if !pending.is_empty() {
                    return Err(AgentError::new(
                        ErrorCode::ModelProtocol,
                        "input interleaves an unsettled tool batch",
                    ));
                }
            }
        }
