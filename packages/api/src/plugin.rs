use crate::{
    AgentError, CheckpointSink, ContextTransform, ErrorCode, EventObserver, Model, Result,
    ResultTransform, Tool, ToolPolicy, ToolSelector, API_VERSION,
};
...
impl Services {
    ...
    pub fn get<T: Any + Send + Sync>(&self, key: &str) -> Result<Arc<T>> {
        let value = self.entries.get(key).ok_or_else(|| {
            AgentError::new(ErrorCode::Dependency, format!("service missing: {key}"))
        })?;
        value.clone().downcast::<T>().map_err(|_| {
            AgentError::new(
                ErrorCode::Dependency,
                format!("service type mismatch: {key}"),
            )
        })
    }
    pub fn insert(&mut self, service: ServiceRegistration) -> Result<()> {
        if self.entries.contains_key(&service.key) {
            return Err(AgentError::new(
                ErrorCode::Dependency,
                format!("duplicate service: {}", service.key),
            ));
        }
