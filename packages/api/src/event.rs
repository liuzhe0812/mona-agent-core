use crate::{
    clip_utf8, AgentError, Result, RunStatus, TaskUsage, ToolCall, ToolResult, ToolStatus,
};
...
impl ItemState {
    ...
    pub fn from_tool(status: ToolStatus) -> Self {
        match status {
            ToolStatus::Success => Self::Completed,
            ToolStatus::Error => Self::Failed,
            ToolStatus::Denied => Self::Denied,
            ToolStatus::Skipped => Self::Skipped,
...
impl WorkItem {
    pub fn message(step: usize) -> Self {
        Self {
            id: message_item_id(step),
            step,
            state: ItemState::Running,
            content: ItemContent::AgentMessage {
                text: String::new(),
                truncated: false,
            },
        }
    ...
    pub fn set_call(&mut self, call: &ToolCall) {
        ...
        } = &mut self.content
        {
            ...
            *arguments = if *arguments_truncated {
                None
            } else {
...
pub fn message_item_id(step: usize) -> String {
    format!("step-{step}-message")
}
pub fn tool_item_id(step: usize, index: usize) -> String {
    format!("step-{step}-tool-{index}")
}
...
impl RunSnapshot {
    ...
    pub fn apply(&mut self, envelope: &EventEnvelope) -> bool {
        if envelope.protocol_version != STREAM_VERSION
            || envelope.run_id != self.run_id
            || envelope.seq != self.seq + 1
        {
            return false;
        }
        ...
        match &envelope.event {
            ...
            RunEvent::ItemStarted { item }
            | RunEvent::ItemUpdated { item }
            | RunEvent::ItemCompleted { item } => {
            ...
            RunEvent::ToolArgumentsDelta {
                ...
                }) = self.items.iter_mut().find(|i| &i.id == item_id)
                {
                    ...
                    if let Some(new_name) = new_name {
                        *name = clip_utf8(new_name, 128).to_owned();
                    }
...
pub trait EventObserver: Send + Sync {
    ...
    async fn on_lagged(&self, _run_id: &str, _lost: u64) -> Result<()> {
        Ok(())
    }
}
