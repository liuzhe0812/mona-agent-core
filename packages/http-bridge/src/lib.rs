...
async fn info() -> Json<serde_json::Value> {
    Json(serde_json::json!({"protocol_version": api::STREAM_VERSION,
...
async fn snapshot(State(state): State<BridgeState>, Path(id): Path<String>) -> std::result::Result<Json<api::RunSnapshot>, HttpError> {
