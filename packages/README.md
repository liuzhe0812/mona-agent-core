...
| [application](application/README.md) | 任务调用管理、可信历史接纳、幂等、订阅、回放和保留策略 | 界面与传输的共享调用层；持久会话由宿主拥有，见 [统一应用接口](../docs/APPLICATION-API.zh-CN.md) |
...
正式 Server 的四个基础工具固定提供，三个检索工具按配置追加；Skills 默认关闭，compaction/spill 默认启用。项目规则 `instructions` 是默认启用、可关闭的宿主模块；上下文来源预算、压缩状态及会话归档之间的边界见[上下文管理](../docs/CONTEXT-MANAGEMENT.zh-CN.md)。包本身的接口与正式宿主的装配策略分开维护，配置来源、覆盖规则和重启生效行为见 [能力装配与管理](../docs/CAPABILITY-ASSEMBLY.zh-CN.md)。
