...
use api::{AgentRuntime, Message, ModelOptions, RunEvent, RunLimits, RunRequest, RunSession, RunSnapshot, TaskControl, TaskLimits};
...
pub(crate) struct RunData {
    ...
    pub journal: VecDeque<(api::EventEnvelope, usize)>,
    pub journal_bytes: usize,
    pub reset_floor: u64,
...
impl Entry {
    ...
    fn accept(&self, event: api::EventEnvelope, config: &ApplicationConfig) {
...
async fn collect(entry: Arc<Entry>, mut events: broadcast::Receiver<api::EventEnvelope>, config: ApplicationConfig) {
    let session = entry.session.clone();
