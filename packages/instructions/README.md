...
从指定工作空间根开始，每目录优先 AGENTS.md，不存在时用 CLAUDE.md。结构化 `read/write/edit/grep/find/ls` 路径触发相关目录及祖先规则，不递归扫描整个仓库。每轮重新读取适用文件，来源不会混入正式用户历史。副作用工具发现新规则或规则已改变时，拒绝该次未执行调用，让模型获得新规则后重新发起；同批工具之间也检查，拒绝不表示撤销旧动作。
...
当前 Web 默认装配此扩展，部署开关仍由产品层管理。其他宿主按场景装配自己的规则内容；该包不将通用 Agent 固定为编程助手。
